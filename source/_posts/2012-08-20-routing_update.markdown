...
- **Line 3** sets the location property of the **Router** to **hash** which means we will be using url fragment identifiers like **#/post/1** for routing.  Ember does not support <a href="http://badassjs.com/post/840846392/location-hash-is-dead-long-live-html5-pushstate" target="_blank">pushstate</a> at this time of writing but I expect that to change in due course.
...
When an Ember application loads, Ember will parse the URL and attempt to find and Ember.Route within the Router's child route hierarchy that matches.  Loading the page at '/' will detect a route property of **root.index.home**.  What this means is that the router will first of all transition to the **root** route and then to the index
